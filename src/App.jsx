...
.auth-wrap{
  ...
  background:radial-gradient(ellipse 80% 60% at 50% 0%, rgba(60,219,192,.1) 0%, transparent 70%),
             radial-gradient(ellipse 40% 40% at 80% 80%, rgba(60,219,192,.04) 0%, transparent 60%),
             var(--bg);
  padding:24px;
}
.auth-box{
  width:100%;max-width:420px;
  background:rgba(26,27,39,.96);
...
.auth-mark{width:36px;height:36px;background:#3CDBC0;display:flex;align-items:center;justify-content:center;font-family:'Montserrat',sans-serif;font-size:18px;font-weight:800;color:#13131c;border-radius:8px}
...
.btn-primary{
  width:100%;padding:13px;background:#3CDBC0;border:none;color:#13131c;
  font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;letter-spacing:.5px;
  cursor:pointer;transition:.15s;text-transform:uppercase;border-radius:8px;
}
...
.topbar-mark{width:32px;height:32px;background:#3CDBC0;display:flex;align-items:center;justify-content:center;font-family:'Montserrat',sans-serif;font-size:16px;font-weight:800;color:#13131c;flex-shrink:0;border-radius:8px}
.topbar-name{font-family:'Montserrat',sans-serif;font-size:14px;font-weight:800;color:#fff;letter-spacing:.3px}
...
.btn-logout{padding:7px 16px;background:rgba(255,255,255,.05);border:1px solid var(--border);color:var(--muted);font-size:12px;font-weight:600;cursor:pointer;transition:.15s;letter-spacing:.3px;border-radius:6px}
.btn-logout:hover{border-color:var(--border2);color:var(--text);background:rgba(255,255,255,.08)}
...
.sidebar.collapsed .snav-item:hover::after{content:attr(data-label);position:absolute;left:54px;top:50%;transform:translateY(-50%);background:#1a1b27;border:1px solid rgba(255,255,255,.15);color:#eef2ff;font-size:11px;font-weight:600;padding:5px 12px;border-radius:6px;white-space:nowrap;z-index:9999;pointer-events:none;box-shadow:0 4px 16px rgba(0,0,0,.5)}
...
.stat-card{background:var(--card);border:1px solid var(--border);padding:18px 20px;display:flex;flex-direction:column;gap:6px;border-radius:12px;transition:border-color .15s}
...
.btn-sm{padding:5px 12px;font-size:12px;font-weight:600;border:1px solid;cursor:pointer;transition:.15s;letter-spacing:.2px;border-radius:6px}
...
.modal-box{background:var(--card);border:1px solid var(--border2);width:100%;max-width:460px;max-height:90vh;overflow-y:auto;animation:fadeUp .25s ease;border-radius:14px;box-shadow:0 24px 64px rgba(0,0,0,.5)}
...
.btn-cancel{padding:9px 18px;background:rgba(255,255,255,.05);border:1px solid var(--border);color:var(--muted);font-size:13px;font-weight:600;cursor:pointer;transition:.15s;border-radius:7px}
.btn-cancel:hover{color:var(--text);border-color:var(--border2);background:rgba(255,255,255,.08)}
.btn-confirm{padding:9px 20px;background:#3CDBC0;border:none;color:#13131c;font-family:'Montserrat',sans-serif;font-size:13px;font-weight:700;cursor:pointer;transition:.15s;letter-spacing:.3px;border-radius:7px}
...
/* ── WELCOME ── */
.welcome-card{background:linear-gradient(135deg,rgba(60,219,192,.12),rgba(60,219,192,.03));border:1px solid rgba(60,219,192,.2);padding:24px 28px;margin-bottom:24px;border-radius:12px}
...
/* ── PERFIS DINÂMICOS ── */
.pfcard{background:var(--card);border:1px solid var(--border);padding:18px 20px;border-radius:12px;display:flex;flex-direction:column;gap:10px;transition:border-color .15s}
...
const PRODUTOS = [
  ...
  {id:"nb12", cat:"informatica", ncm:"8471.30.12", nome:'Notebook/Tablet 8"-14"',        cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:2.18, bkpPad:2.54, mva:40,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:7,  icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  {id:"nb19", cat:"informatica", ncm:"8471.30.19", nome:'Notebook/Tablet 15"+',          cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:2.18, bkpPad:2.54, mva:40,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:7,  icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  {id:"tab7", cat:"informatica", ncm:"8471.30.11", nome:'Tablet 7"',                     cfixoPad:4.76, royalPad:2.49, scrapPad:0.91, fretePad:1.70, bkpPad:2.54, mva:40,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:12, icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  {id:"cpu",  cat:"informatica", ncm:"8471.50.10", nome:"CPU Pequena Capacidade",        cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:1.65, bkpPad:2.54, mva:36,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:9.75, credMAO:12, credIOS:12, credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"aio",  cat:"informatica", ncm:"8471.49.00", nome:"All In One / Servidor",         cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:1.65, bkpPad:2.54, mva:30,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:9.75, ipiCWB:9.75, credMAO:12, credIOS:12, credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"smt",  cat:"smartphones", ncm:"8517.13.00", nome:"Smartphone",                    cfixoPad:4.76, royalPad:2.01, scrapPad:1.12, fretePad:0.72, bkpPad:1.50, mva:28,  aliqST:12,  fti:2.2, ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"fp",   cat:"smartphones", ncm:"8517.14.31", nome:"Feature Phone (linha P)",       cfixoPad:2.00, royalPad:0,    scrapPad:0.51, fretePad:1.54, bkpPad:1.50, mva:28,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:11.25,ipiCWB:11.25,credMAO:12, credIOS:0,  credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"vpc",  cat:"smarthome",   ncm:"8517.62.77", nome:"Smart Video Porteiro",          cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.48, mva:66,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  {id:"rtr",  cat:"smarthome",   ncm:"8517.62.41", nome:"Router Mesh",                   cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.48, mva:42,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"gw",   cat:"smarthome",   ncm:"8517.62.94", nome:"Smart Central/Gateway",         cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.48, mva:31,  aliqST:12,  fti:0,   ipiMAO:0,    ipiIOS:9.75, ipiCWB:9.75, credMAO:12, credIOS:0,  credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"cam",  cat:"smarthome",   ncm:"8525.89.29", nome:"Smart Camera WiFi",             cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.48, mva:64,  aliqST:18,  fti:2.2, ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  ...
  {id:"kbd",  cat:"informatica", ncm:"8471.60.52", nome:"Teclado Importação Direta",     cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:2.18, bkpPad:2.54, mva:40,  aliqST:12,  fti:0,   ipiMAO:9.75, ipiIOS:9.75, ipiCWB:9.75, credMAO:3,  credIOS:4,  credCWB:4,  icmsMAO:4,  icmsIOS:12, icmsCWB:4 },
  ...
  {id:"spk",  cat:"smarthome",   ncm:"8518.22.00", nome:"Caixa de Som Bluetooth",        cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.48, mva:41,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"chr",  cat:"mobilidade",  ncm:"8504.40.10", nome:"Carregador Celular",            cfixoPad:4.76, royalPad:0,    scrapPad:0.51, fretePad:1.54, bkpPad:1.50, mva:70,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:5,    ipiCWB:5,    credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"lmp",  cat:"smarthome",   ncm:"8539.52.00", nome:"Smart Lâmpada WiFi",            cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.28, mva:0,    aliqST:0,  fti:0,   ipiMAO:0,    ipiIOS:6.5,  ipiCWB:6.5,  credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"rob",  cat:"smarthome",   ncm:"8508.11.00", nome:"Smart Robô Aspirador",          cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.72, mva:43,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:6.5,  ipiCWB:6.5,  credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4 },
  {id:"spg",  cat:"smarthome",   ncm:"8536.50.90", nome:"Smart Plug WiFi",               cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.76, mva:49,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:3.25, ipiCWB:3.25, credMAO:12, credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:7 },
  // ── SmartHome — iluminação extra ─────────────────────────────────────────
  {id:"lum1", cat:"smarthome",   ncm:"9405.11.90", nome:"Smart Luminária Painel/Embutir",cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:51,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:9.75, ipiCWB:9.75, credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  {id:"lum2", cat:"smarthome",   ncm:"9405.21.00", nome:"Smart Luminária de Mesa",       cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:51,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:9.75, ipiCWB:9.75, credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  {id:"fled", cat:"smarthome",   ncm:"9405.42.00", nome:"Smart Fita LED",                cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:60,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:0,    ipiCWB:9.75, credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  // ── SmartHome — acessórios aspirador / casa ──────────────────────────────
  {id:"rasp", cat:"smarthome",   ncm:"8508.70.00", nome:"Acessório Robô (tanque/filtro)",cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:43,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:6.5,  ipiCWB:6.5,  credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  {id:"petf", cat:"smarthome",   ncm:"8509.80.90", nome:"Alimentador PET / Robô Laser",  cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:38,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:6.5,  ipiCWB:6.5,  credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  {id:"ctrl", cat:"smarthome",   ncm:"8543.70.99", nome:"Sensor / Controle Eletrônico",  cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:1.84, mva:0,    aliqST:0,  fti:0,   ipiMAO:0,    ipiIOS:6.5,  ipiCWB:6.5,  credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  ...
  {id:"lock", cat:"smarthome",   ncm:"8301.40.00", nome:"Fechadura Eletrônica",          cfixoPad:7.14, royalPad:0,    scrapPad:1.09, fretePad:2.45, bkpPad:2.72, mva:85,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:0,    ipiCWB:0,    credMAO:0,  credIOS:3,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  // ── Periféricos / Acessórios ─────────────────────────────────────────────
  {id:"mse",  cat:"informatica", ncm:"8471.60.53", nome:"Mouse",                         cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:2.18, bkpPad:2.54, mva:40,  aliqST:12,  fti:0,   ipiMAO:9.75, ipiIOS:9.75, ipiCWB:9.75, credMAO:3,  credIOS:12, credCWB:4,  icmsMAO:4,  icmsIOS:12, icmsCWB:4  },
  ...
  {id:"mlt",  cat:"informatica", ncm:"4202.92.00", nome:"Maleta para Notebook",          cfixoPad:4.34, royalPad:0,    scrapPad:0.71, fretePad:2.18, bkpPad:2.54, mva:57.24,aliqST:18, fti:0,   ipiMAO:6.5,  ipiIOS:6.5,  ipiCWB:6.5,  credMAO:3,  credIOS:3,  credCWB:0,  icmsMAO:12, icmsIOS:4,  icmsCWB:4  },
  {id:"fone", cat:"mobilidade",  ncm:"8517.62.72", nome:"Fone / Headset Bluetooth",      cfixoPad:4.76, royalPad:0,    scrapPad:0.51, fretePad:1.54, bkpPad:1.50, mva:66,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:12, credIOS:12, credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:4  },
  {id:"cabo", cat:"mobilidade",  ncm:"8544.42.00", nome:"Cabo para Carregador",          cfixoPad:4.76, royalPad:0,    scrapPad:0.51, fretePad:1.54, bkpPad:1.50, mva:43,  aliqST:12,  fti:0,   ipiMAO:0,    ipiIOS:5,    ipiCWB:5,    credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:4  },
  ...
  // ── PosiSeg — CFTV ───────────────────────────────────────────────────────
  {id:"nvr",  cat:"posiseg",     ncm:"8521.90.00", nome:"DVR / XVR / NVR Gravador",     cfixoPad:30.32,royalPad:0,    scrapPad:2.00, fretePad:1.09, bkpPad:1.20, mva:39,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:15,   ipiCWB:15,   credMAO:0,  credIOS:0,  credCWB:0,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
  ...
  {id:"bln",  cat:"posiseg",     ncm:"8504.40.21", nome:"Balun / Power Balun",           cfixoPad:30.32,royalPad:0,    scrapPad:2.00, fretePad:1.09, bkpPad:1.20, mva:56,  aliqST:18,  fti:0,   ipiMAO:0,    ipiIOS:3.75, ipiCWB:3.75, credMAO:0,  credIOS:0,  credCWB:4,  icmsMAO:12, icmsIOS:12, icmsCWB:12},
...
@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800&display=swap');
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
body{background:var(--bg,#13131c);color:#eef2ff;font-family:'Montserrat',Arial,Helvetica,sans-serif;min-height:100vh}
input::-webkit-inner-spin-button,input::-webkit-outer-spin-button{-webkit-appearance:none}
.app{display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden}
.hdr{background:#0d0d15;color:#fff;padding:0 20px;display:flex;align-items:center;gap:16px;min-height:54px;border-bottom:2px solid #3CDBC0;flex-wrap:wrap}
...
.sech{padding:9px 14px;background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.07);display:flex;align-items:center;justify-content:space-between;min-height:34px}
...
.fw{display:flex;align-items:center;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);overflow:hidden;min-width:115px;flex-shrink:0;transition:.15s;border-radius:6px}
...
.fpre{padding:0 8px;font-family:'Montserrat',sans-serif;font-size:10px;color:#7a8ba3;background:rgba(255,255,255,.03);border-right:1px solid rgba(255,255,255,.07);white-space:nowrap;align-self:stretch;display:flex;align-items:center}
.fsel{border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);padding:7px 8px;font-family:'Montserrat',sans-serif;font-size:11px;color:#eef2ff;outline:none;min-width:115px;flex-shrink:0;cursor:pointer;border-radius:6px}
.fsel:focus{border-color:#3CDBC0}
.cbtn{width:28px;height:28px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);cursor:pointer;font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:.15s;border-radius:6px;color:#7a8ba3;font-family:'Montserrat',sans-serif}
...
.rgb{flex:1;padding:7px 8px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#7a8ba3;font-family:'Montserrat',sans-serif;font-size:12px;font-weight:500;cursor:pointer;transition:.15s;min-width:60px;border-radius:5px}
...
.pright{display:flex;flex-direction:column;background:#2C2A29;min-height:0;overflow:hidden}
...
.bdc{background:#1a1b27;border:1px solid rgba(255,255,255,.08);border-radius:10px;overflow:hidden}
.bdr{display:flex;align-items:center;padding:4px 8px;gap:6px;border-bottom:1px solid rgba(255,255,255,.04)}
...
.bd-grp{background:#1a1b27;border:1px solid rgba(255,255,255,.08);border-radius:8px;overflow:hidden}
...
.bd-tot{display:flex;align-items:center;gap:4px;padding:7px 8px;background:rgba(255,255,255,.04);border-radius:6px}
...
.bd-preco{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:linear-gradient(135deg,rgba(60,219,192,.12),rgba(60,219,192,.05));border:1px solid rgba(60,219,192,.3);border-radius:10px}
...
/* ── CARD BRANCO — estilo Positivo sobre fundo escuro ── */
.card-white{background:#f0f4ff;border:1px solid rgba(255,255,255,.12);border-radius:12px;color:#1a1b2e}
.card-white .stat-label{color:#5a6a84}
.card-white .stat-val{color:#1a1b2e}
.card-white .stat-sub{color:#7a8ba3}
...
- ⚠BUSCA DE PRODUTO — NUNCA INVENTE NOMES: se buscar_produto não achar o produto exato, tente de novo com termos mais simples/parciais (ex: só "Vision C15" ou só "C15", sem quantidade de memória) antes de desistir. Ao oferecer opções via perguntar_usuario para o usuário escolher o produto, as opções DEVEM ser exatamente os "nome" retornados por uma chamada real a buscar_produto — NUNCA invente ou "poliça" variações de nome que pareçam plausíveis mas não vieram da base. Se nenhuma chamada a buscar_produto retornar nada, diga isso honestamente ao usuário e peça o ID/nome exato do Cadastro, em vez de sugerir nomes que podem não existir.
- ⚠⚠LISTA DE PRODUTOS PRA ESCOLHER = SEMPRE perguntar_usuario, NUNCA texto puro: sempre que houver 2+ candidatos pra usuário escolher (ex.: "quais destes têm VPL cadastrado?"), apresente as opções via perguntar_usuario (com os "nome"/"id" exatos de buscar_produto), NUNCA como uma lista em texto solto esperando o usuário responder "a terceira"/"o segundo". Já aconteceu de o usuário responder por ordinal a uma lista em texto e o agente escolher o SKU errado (recontando a lista de cabeça e errando). Se o usuário mesmo assim responder por ordinal/posição a uma lista já mostrada, releia a MESMA lista anterior com cuidado antes de chamar set_produto — nunca "lembre" um SKU parecido de memória.
...
const CALC_TOOLS = [
  ...
  { type:"function", function:{ name:"perguntar_usuario", description:"Use SEMPRE que precisar de uma informação que não está na base de dados nem foi dita pelo usuário (ex: UF de destino, canal/cliente, prazo de pagamento). NUNCA assuma um valor (ex: nunca assuma São Paulo como UF de destino) — pergunte de forma interativa com opções. O usuário verá botões com as opções e um campo para digitar outro valor. ⚠NUNCA escreva a pergunta como texto normal na resposta — SEMPRE chame esta ferramenta, uma pergunta por vez (uma de cada vez, não numere várias perguntas num único texto). Se precisar de várias informações, chame esta ferramenta várias vezes (uma por informação) — cada uma será exibida em sequência. ⚠QUANDO A PERGUNTA FOR SOBRE QUAL PRODUTO: as opções têm que ser o campo 'nome' exato retornado por uma chamada real a buscar_produto — NUNCA um nome inventado/reformulado que pareça plausível.",
    parameters:{ type:"object", properties:{
...
function Calculadora({user:currentUser, isAdmin=false, nomeAba="", onRenomear=null, onCalcsChange=null}){
  ...
  const c=useMemo(()=>{
    ...
    const custoFinVf=pFbase*((d.custoFin||0)/100),custoFixoCanVf=pFbase*((d.custoFixoCan||0)/100);
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <Sec title="Margem Líquida (ML)" hl>
                  ...
                  <DR label={`MC = ML${d.margGerAtivo&&d.margGer!==0?" + Margem Gerencial/Agnóstica":""}`} value={pct(c.mc)} bold accent="blue"/>
